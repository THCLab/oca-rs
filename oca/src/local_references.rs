...
pub fn dereference_ast(oca_ast: &mut OCAAst, references: HashMap<String, String>) {
                    ...
                    match attr_type {
                        NestedAttrType::Reference(RefValue::Name(refn)) => {
                            if let Some(said) = references.get(refn) {
                                *attr_type = NestedAttrType::Reference(RefValue::Said(said.to_string()));
                            } else {
                        ...
                        NestedAttrType::Array(box_attr_type) => {
                            if let NestedAttrType::Reference(RefValue::Name(refn)) = &**box_attr_type {
                                if let Some(said) = references.get(refn) {
                                    **box_attr_type = NestedAttrType::Reference(RefValue::Said(said.to_string()));
                                } else {
                                    panic!("Reference not found: {}", refn);
                                }
                            }
